...
class MainScreen(QDialog):
    ...
    def search_food_handle(self, new_text):
        ...
        for data in self.market.SearchFood(new_text, date):
            if data["INVENTORY"] < 1:
                continue
            checkbox = QCheckBox(
                f"{data['NAME']}\t {data['PRICE']}$", objectName=str(data["ID"])
            )
    ...
    def update_news(self):
        ...
        if not len(data) == 0:
            for news in data:
                ...
                label.setStyleSheet(
                    'QLabel { font: 12pt "MV Boli"; min-height: 30px; max-height: 20px;}'
                )
    ...
    def update_vote(self):
        ...
        try:
            for data in self.market.FoodMenu(date):
                ...
                radio_button.setStyleSheet(
                    'QRadioButton { font: 12pt "MV Boli"; min-height: 20px; }'
                )
        ...
        try:
            ...
            for data in all_data:
                ...
                self.foods_orderd.addItem(
                    f"{data['FOOD_ID']} - {data['ID']} - {data_food['NAME']} - {data_food['DATE']}"
                )
        ...
        if radioButton.isChecked():
            ...
            try:
                ...
                for data in all_data:
                    ...
                    label.setStyleSheet(
                        'QLabel { font: 10pt "MV Boli"; min-height: 20px; }'
                    )
    ...
    def use_copon_handle(self):
        ...
        if status == False:
            ...
            if msg["COUNT"] > 0:
                new_total = int(
                    self.CartTotalAmount.intValue() * (100 - msg["PERCENT"]) / 100
                )
    ...
    def pay_handle(self):
        try:
            ...
            if self.discount_code.isEnabled() or self.discount_code.text() == "":
                ...
                self.user.Pay(
                    self.CartTotalAmount.intValue(), date, self.discount_code.text()
                )
            self.update_cart()
            self.update_orders()
